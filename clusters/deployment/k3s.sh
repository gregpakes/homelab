...
master2=172.16.51.202
master3=172.16.51.203
worker1=172.16.51.204
worker2=172.16.51.205
...
lbrange=172.16.51.60-172.16.51.80
...
SSH_OPTS="-i $HOME/.ssh/$certName -o BatchMode=yes -o StrictHostKeyChecking=no"
for node in "${all[@]}"; do
  if ssh $SSH_OPTS "$user@$node" true 2>/dev/null; then
    echo "Key auth already works on $node — skipping ssh-copy-id"
  else
    echo "Installing key on $node…"
    ssh-copy-id -i "$HOME/.ssh/$certName.pub" "$user@$node"
  fi
done
